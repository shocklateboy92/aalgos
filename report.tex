...
\begin{abstract}
% % K-linearizability has proven to be a highly effective way to increase the performance
...
its effectiveness by relaxing the push\_back() operation to write
...
(feature-equivalent) vector against the vector from intel\textbf{'}s
industry leading thread building blocks library and show it is faster
by a factor of X.
\end{abstract}
...
K-linearizability has already been used to increase the throughput and scalability of FIFO queues by turning them into k-FIFO queues[2]; LIFO stacks to k-LIFO stacks, and so on. The process for turning most data structures to thier k-linearizable equivalents has been quite straightforward for these data structures: instantiate multiple copies of a linearizable implementation and distribute operations between individual instances with the use of a load balancer[4]. This has the drawback that it cannot be used for a data structure that requires it's elements to be in a contiguous block of memory, such as a vector.

Here we present a vector that is lock-free vector with a very scalable high throughput push\_back operation. This is the only k-linearizabile operation in the vector and it inserts an element anywhere within the bounds of the \emph{k-region}. The k-region begins at the \emph{hard-tail} (before which all elements are guaranteed to be contiguous), and is exactly k-elements long. It also supports random access (read, write), reserve and size operations. Note that it does not support pop\_back even though it is considered standard for a vector, for performance reasons.
...
However, it has some limitations \citep{dechev06}.
