...
1) Immediately after the linearization of a \op{push\_back} operation, if the value of \op{vector.tail} is $t$, then the operation will insert into an index $i$ with $t \le i \le t + k$. Sketch proof: the CAS operation that performs the linearized insert in \op{push\_back} uses $t$ plus an offset, however the offset is between 0 and $k$ 

2) Immediately after the $p$th linearization of a \op{push\_back} operation, the value of \op{vector.tail} is between $p-k$ and $p$. Sketch proof: the value of \op{vector.tail} is changed only by calls to \op{advance\_tail}, which increases it to the index of the first sentinel value. Since elements are only added within 
